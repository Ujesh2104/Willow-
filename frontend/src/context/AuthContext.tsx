...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      localStorage.removeItem('willow_cached_users');
    } catch {}
  }, []);
  ...
  const login = async (email: string, password?: string): Promise<{ success: boolean; user?: User; message?: string }> => {
    ...
    try {
      ...
      return { 
        success: false, 
        message: res?.message || `User Not Found: No account is registered with ${sanitized}. Please click "Create Account" below.` 
      };
    } catch (err: any) {
      return { 
        success: false, 
        message: err?.message || `User Not Found: No account is registered with ${sanitized}. Please click "Create Account" below.` 
      };
    }
  };
  ...
  const register = async (name: string, email: string, phone: string, password?: string): Promise<{ success: boolean; message?: string }> => {
    try {
      ...
      return { 
        success: false, 
        message: err?.message || 'Server error during registration. Please try again.' 
      };
    }
  };
  ...
  const logout = async () => {
    ...
    // Clear all client session state immediately
    setUser(null);
    setToken(null);
    try {
      localStorage.removeItem('willow_active_user');
    ...
    // Invalidate session on backend server
